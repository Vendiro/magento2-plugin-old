...
if [ -z $MAGENTO_VERSION ]; then MAGENTO_VERSION="2.2.9"; fi
...
if [ ! -f "$CACHE_FILE" ]; then
    wget "http://magento.mirror.hypernode.com/releases/magento2-${MAGENTO_VERSION}.tar.gz" -O $CACHE_FILE
fi
...
REPOSITORY_CONFIG="{\"type\": \"package\",\"package\": { \"name\": \"tig/vendiro_travis\", \"version\": \"master\", \"dist\": {\"type\": \"zip\",\"url\": \"${TRAVIS_BUILD_DIR}/build.zip\",\"reference\": \"master\" }, \"autoload\": {\"files\": [\"registration.php\"],\"psr-4\": {\"TIG\\\\Vendiro\\\\\": \"\"}}}}"
...
( cd "${BUILD_DIR}/" && composer config repositories.vendiro "${REPOSITORY_CONFIG}" )
...
( cd "${BUILD_DIR}/" && composer require tig/vendiro_travis --ignore-platform-reqs )
...
mysql -u${MAGENTO_DB_USER} ${MYSQLPASS} -h${MAGENTO_DB_HOST} -P${MAGENTO_DB_PORT} ${MAGENTO_DB_NAME} < Test/Fixtures/tig-vendiro-fixture.sql
